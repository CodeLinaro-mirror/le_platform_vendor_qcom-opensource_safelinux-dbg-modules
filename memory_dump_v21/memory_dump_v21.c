...
/* New Dump table version to support more than 16 cores. */
#define MSM_DUMP_TABLE_VERSION		MSM_DUMP_MAKE_VERSION(2, 1)
...
#define SPR_DUMP_CPU8			0x1F8
#define SPR_DUMP_CPU9			0x1F9
#define SPR_DUMP_CPU10			0x1FA
#define SPR_DUMP_CPU11			0x1FB
#define SPR_DUMP_CPU12			0x1FC
#define SPR_DUMP_CPU13			0x1FD
#define SPR_DUMP_CPU14			0x1FE
#define SPR_DUMP_CPU15			0x1FF
#define SPR_DUMP_CPU16			0x200
#define SPR_DUMP_CPU17			0x201
...
static void mem_dump_parse_register_entry(struct memdump_info *dump_info)
{
	...
	for_each_available_child_of_node(dump_info->dev_node, child_node) {
		...
		ret = of_property_read_u32(child_node, "qcom,instance", &instance);
		if (ret) {
			dev_dbg(dump_info->dev, "Unable to find instance id for %s\n",
					child_node->name);
			instance = 0;
		}
		...
		if ((id == CPUSS_REGDUMP) ||
				((id >= SPR_DUMP_CPU0) && (id <= SPR_DUMP_CPU17)))
...
static int mem_dump_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to set DMA properties\n");
		return ret;
	}
